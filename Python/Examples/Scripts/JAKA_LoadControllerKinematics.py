...
if len(sys.argv) > 1 and os.path.exists(sys.argv[1]):
    ...
    fileopen_urp = filedialog.askopenfilename(initialdir=rdk_path, title="Select a URP file generated using Polyscope on the real robot (not generated using RoboDK or UR Simulator)", filetypes=(("JAKA files", "*.txt"), ("all files", "*.*")))
    if not fileopen_urp:
        quit()

# Read the compressed URP file
urp_xml = None
with open(fileopen_urp, 'rb') as fid:
    urp_xml = fid.read()
...
id_1 = urp_xml.index('<jaka_dh')
id_2 = urp_xml.index('</jaka_dh')
    ...
    for child in xml:
        if child.tag == 'alpha':
            if len(child.attrib) == 0:
                ...
                dh_alpha = []
                for d in child:
                    dh_alpha.append(float(d.text) * pi / 180)
            else:
                value = child.attrib['value']
                dh_alpha = [float(v) * pi / 180 for v in value.replace(' ', '').split(',')]
        ...
        if child.tag == 'a':
            if len(child.attrib) == 0:
                ...
                dh_a = []
                for d in child:
                    dh_a.append(float(d.text))
            else:
                value = child.attrib['value']
                dh_a = [float(v) for v in value.replace(' ', '').split(',')]
        ...
        if child.tag == 'deltaTheta':
            if len(child.attrib) == 0:
                ...
                dh_dTheta = []
                for d in child:
                    dh_dTheta.append(float(d.text) * pi / 180)
            else:
                value = child.attrib['value']
                dh_dTheta = [float(v) * pi / 180 for v in value.replace(' ', '').split(',')]
        ...
        if child.tag == 'd':
            if len(child.attrib) == 0:
                ...
                dh_d = []
                for d in child:
                    dh_d.append(float(d.text))
            else:
                value = child.attrib['value']
                dh_d = [float(v) for v in value.replace(' ', '').split(',')]
