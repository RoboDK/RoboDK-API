...
namespace RoboDk.API
{
    ...
    internal class Item : IItem
    {
        ...
        /// <param name="parentTypes">The parent allowed types, such as ItemType.Frame, defaults to null</param>
        /// <returns>A list of parents, ordered from the Item's parent to the Station.</returns>
        public static List<IItem> GetAncestors(IItem item, List<ItemType> parentTypes = null)
        {
            List<IItem> result = new List<IItem>();

            if (parentTypes == null)
            {
                parentTypes = new List<ItemType>();
            }

            IItem parent = item;
            while (parent.Valid() && parent.GetItemType() != ItemType.Station && parent.GetItemType() != ItemType.Any)
            {
                ...
                {
                    if (parent.GetItemType() == parentType)
                    {
                        result.Add(parent);
                        break;
                    }
        ...
        public static Mat GetAncestorPose(IItem child, IItem parent)
        {
            ...
            List<IItem> items = GetAncestors(child);
            items.Insert(0, child);

            int index = -1;
            for (int i = 0; i < items.Count; i++)
            {
                if (items[i].ItemId == parent.ItemId)
                {
                    index = i;
                    break;
                }
            ...
            {
                if (items[i].GetItemType() == ItemType.Tool)
                {
                    poses.Add(items[i].PoseTool());
                }
                else if (items[i].GetItemType() == ItemType.Robot)
                {
                    if (items[i].GetLink(ItemType.Robot).ItemId != items[i].ItemId)
                    {
                        continue;
                    }

                    List<IItem> links = items[i].GetLinks(ItemType.RobotAxes);
                    if (links.Count > 0)
                    {
        ...
        /// <inheritdoc />
        public Mat GetPoseWrt(IItem item)
        {
            ...
            if (this.GetItemType() == ItemType.Robot || this.GetItemType() == ItemType.Tool)
            {
                pose1 = GetAncestorPose(this, this.RDK().GetActiveStation());
            }

            if (item.GetItemType() == ItemType.Robot || item.GetItemType() == ItemType.Tool)
            {
