...
public class RoboDK
{
    ...
    public class Item
    {
        ...
        /// <summary>
        ///     Get the list of parents of an Item up to the Station, with type filtering (i.e. ItemType.Frame, ItemType.Robot, ..).
        ...
        /// <param name="parent_types">The parent allowed types, such as ItemType.Frame, defaults to null</param>
        /// <returns>A list of parents, ordered from the Item's parent to the Station.</returns>
        public static List<Item> getAncestors(Item item, List<int> parent_types = null)
        {
            List<Item> result = new List<Item>();

            if (parent_types == null)
            {
                parent_types = new List<int>();
            }

            Item parent = item;
            while (parent.Valid() && parent.Type() != ITEM_TYPE_STATION && parent.Type() != ITEM_TYPE_ANY)
            {
                parent = parent.Parent();

                if (parent_types.Count < 1)
                {
                    result.Add(parent);
                    continue;
                }

                foreach (var parentType in parent_types)
                {
        ...
        /// </summary>
        /// <param name="item_child">The child Item</param>
        /// <param name="item_parent">The parent Item</param>
        /// <returns>Returns the pose from the child to the parent.</returns>
        public static Mat getAncestorPose(Item item_child, Item item_parent)
        {
            if (item_child.get_item() == item_parent.get_item())
            {
                return Mat.Identity4x4();
            }

            List<Item> items = getAncestors(item_child);
            items.Insert(0, item_child);

            int index = -1;
            for (int i = 0; i < items.Count; i++)
            {
                if (items[i].get_item() == item_parent.get_item())
                {
                    index = i;
                    break;
                }
            ...
            for (int i = index - 1; i >= 0; i--)
            {
                ...
                {
                    poses.Add(items[i].PoseTool());
                }
                else if (items[i].Type() == ITEM_TYPE_ROBOT)
                {
                    if (items[i].getLink(ITEM_TYPE_ROBOT).get_item() != items[i].get_item())
                    {
                        continue;
                    }

                    List<Item> links = items[i].getLinks(ITEM_TYPE_ROBOT_AXES);
                    if (links.Count > 0)
                    {
                        throw new RDKException("This function does not support synchronized axis");
                    }

                    poses.Add(items[i].SolveFK(items[i].Joints()));
                }
                else
                {
                    poses.Add(items[i].Pose());
                }
            }

            Mat result = Mat.Identity4x4();
        ...
        public Mat PoseWrt(Item item)
        {
            if (this.get_item() == item.get_item())
            {
                return Mat.Identity4x4();
            }

            Mat pose1 = this.PoseAbs();
            Mat pose2 = item.PoseAbs();

            if (this.Type() == ITEM_TYPE_ROBOT || this.Type() == ITEM_TYPE_TOOL)
            {
                pose1 = getAncestorPose(this, this.RDK().GetActiveStation());
            }

            if (item.Type() == ITEM_TYPE_ROBOT || item.Type() == ITEM_TYPE_TOOL)
            {
                pose2 = getAncestorPose(item, item.RDK().GetActiveStation());
            }
